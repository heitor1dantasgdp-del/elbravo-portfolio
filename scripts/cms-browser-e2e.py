...
with sync_playwright() as p:
    ...
    for width in (375, 430):
        ...
        check(f'admin mobile {width}px dashboard has no horizontal scroll', lambda: mobile.evaluate('() => { window.scrollTo(1000, 0); return window.scrollX === 0; }'))
        ...
        check(f'admin mobile {width}px media editor has no horizontal scroll', lambda: mobile.evaluate('() => { window.scrollTo(1000, 0); return window.scrollX === 0; }'))
